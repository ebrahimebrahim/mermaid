...
class FourierConvolution(Function):
    ...
    def forward(self, input):
        ...
        if USE_CUDA:
            ...
            f_input = self.fftn(input,self.dim)
            ...
            conv_ouput_real = self.ifftn(f_conv, self.dim,signal_sizes=input.shape[dim_input_batch::])
    ...
    def backward(self, grad_output):
        ...
        if USE_CUDA:
            ...
            f_go = self.fftn(grad_output,self.dim)
            f_filter_real = self.complex_fourier_filter[0]
            ...
            dim_input_batch = dim_input - self.dim
            grad_input = self.ifftn(f_conv,self.dim,signal_sizes=grad_output.shape[dim_input_batch::])
...
class InverseFourierConvolution(Function):
    ...
    def backward(self, grad_output):
        ...
        if USE_CUDA:
            ...
            dim_input = len(grad_output.shape)
            dim_input_batch = dim_input - self.dim
            grad_input = self.ifftn(f_conv, self.dim, signal_sizes=grad_output.shape[dim_input::])
